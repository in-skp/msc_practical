...
@program name: Write a program for Hopfield network model for associative memory
"""
import numpy as np
...
def main():
  ...
  data_file = "C:\\Playground\\msc_practical\\soft_computing\\iris_data_012.txt"
...
(data_x[t] - map[i][j])
  ...
  label_map = np.zeros(shape=(Rows,Cols), dtype=np.int32)
  for i in range(Rows):
    for j in range(Cols):
